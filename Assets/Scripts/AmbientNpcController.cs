...
public class AmbientNpcController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        lastPhysicsPosition = rb != null ? rb.position : (Vector2)transform.position;

        ConfigureRigidbody();
        ConfigureCollisionProbe();
        ResolveOccluderTilemapsIfNeeded();
        TryIgnorePlayerCollisions();
    ...
    private void FixedUpdate()
    {
        if (!isWalking)
        {
            UpdateVisualMovementState(rb != null ? rb.position : (Vector2)transform.position);
            return;
        }
        ...
        {
            BeginIdle(false);
            UpdateVisualMovementState(currentPosition);
            return;
        }
        ...
        Vector2 nextPosition = Vector2.MoveTowards(currentPosition, destination, stepDistance);
        rb.MovePosition(nextPosition);
        UpdateVisualMovementState(nextPosition);
    }
    ...
    {
        SpriteRenderer fallbackRenderer = GetComponent<SpriteRenderer>();
    ...
    private void UpdateSprite()
    {
        ...
        bool shouldAnimateWalk = isWalking || isVisuallyMoving;
        float cycleTime = shouldAnimateWalk ? (Time.time + animationTimeOffset) * animationFramesPerSecond : 0f;
        Sprite nextSprite = visualProfile.Evaluate(facingDirection, shouldAnimateWalk, cycleTime);
        if (nextSprite != null)
            spriteRenderer.sprite = nextSprite;
    }

    private void UpdateVisualMovementState(Vector2 currentPosition)
    {
        float movedDistance = Vector2.Distance(lastPhysicsPosition, currentPosition);
        isVisuallyMoving = movedDistance > 0.0005f;

        if (isVisuallyMoving)
        {
            Vector2 movementDelta = currentPosition - lastPhysicsPosition;
            if (movementDelta.sqrMagnitude > 0.0000001f)
                facingDirection = ResolveCardinalDirection(movementDelta);
        }

        lastPhysicsPosition = currentPosition;
    }

    private void TryIgnorePlayerCollisions()
    {
        ...
        {
            Collider2D[] playerColliders = players[playerIndex].GetComponentsInChildren<Collider2D>(true);
            for (int colliderIndex = 0; colliderIndex < playerColliders.Length; colliderIndex++)
            {
                Collider2D playerCollider = playerColliders[colliderIndex];
                if (playerCollider == null)
                    continue;

                Physics2D.IgnoreCollision(movementCollider, playerCollider, true);
                ignoredAny = true;
            }
        }

        playerCollisionIgnored = ignoredAny;
    }
